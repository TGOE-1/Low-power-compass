...
* [一、项目介绍](#一项目介绍)
* [二、材料清单](#二材料清单)
* [三、软硬件介绍与准备](#三软硬件介绍与准备)
    * [1. 晶振](#1晶振)
    * [2. 磁传感器](#2磁传感器)
    * [3. 点阵屏](#3点阵屏)
    * [4. 杜邦线](#4杜邦线)
    * [5. USB转TTL转换头](#5usb转ttl转换头)
    * [6. 面包板](#6面包板)
    * [7. 光敏模组](#7光敏模组)
    * [8. 单片机](#8单片机)
    * [9. STC-ISP烧录软件](#9stc-isp烧录软件)
* [四、硬件的安装与检测](#四硬件的安装与检测)
    * [1. 最小电路系统](#1最小电路系统)
    * [2. 硬件检测手段——程序烧录](#2硬件检测手段程序烧录)
    * [3. 点阵屏](#3点阵屏连接)
    * [4. 磁传感模块](#4磁传感模块连接)
    * [5. 光敏模块](#5光敏模块连接)
* [五、程序烧录与现象](#五程序烧录与现象)
...
磁力计传感器是整个系统的‘感知核心’。它如同项目的‘眼睛’，负责实时检测地球磁场的方向和强度，并将这些物理量转换为微弱的电信号，为后续计算方位提供最原始的数据基础。它主要提供X，Y，Z三个维度的数据。注意，传感器芯片分为qmc5883l与qmc5883p。目前由于qmc5883l停产，可能商家标注的“qmc5883l”芯片实际上是qmc5883p芯片。如果出现磁传感器异常，尝试更换代码烧录。
...
将单片机放置于面包板上，引脚插入面包板的引脚孔中，位置尽量居中。注意面包板工作区两侧写有数字标记的行数，我们可以将单片机的半圆缺口向上，将第一排引脚插入面包板的第一行，这样方便后续安装各个引脚。如图所示。

![s2-2](./picture/step-2-2.jpg)
...
TXD引脚负责输出数据，RXD引脚负责输入数据，故应将转换头的TXD与单片机RXD，转换头RXD与单片机TXD相连。注意，转换头有三种输出引脚，其中的vcc引脚电压与厂家制作有关，而我们的单片机是标准的5V单片机，因此转换头的输电引脚使用5V。以下是引脚连接表与示例图：
...
![s8-2](./picture/step8-2.png)

### 2.硬件检测手段——程序烧录

最小电路系统安装完后，就可以进行测试了，测试主要通过软件的手段进行，因此这里先介绍如何烧录软件。

(1) 左键双击stc软件，后续的弹框直接点确定。

![stc1](./picture/stc1.png)

(2) 找到stc界面中的单片机型号一栏，在列表中选择STC89C52RC/LE52RC（根据自己单片机的型号选择，此处仅为展示），此时将单片机上的转换器插入电脑USB接口，软件会自动选择串口号。如果未识别串口号，可能是缺少驱动，去官网下载CH341SER.EXE驱动。
...
(4) 选择之前下载的代码中的“测试单片机代码”，然后左键单击打开。
...
(7) 右下角如图所示代表程序烧录成功，此时证明这个最小系统电路没问题。
...
![s6](./picture/点阵屏示例-2.png)

#### （2）点阵屏检测

烧录软件目录中的“点阵屏检测程序”。
代码烧录后，屏幕将会经历“全亮，全灭，逐行点亮后熄灭，亮暗呈棋盘状”四个阶段。
请验证无误后继续下一步。
...
烧录软件目录中的“磁传感检测程序”。注意，由于QMC5883L芯片停产，目前市面上标注的QMC5338L芯片可能实际上是QMC5883P芯片，可以使用放大镜观察芯片上的芯片型号，以选择相应的程序。
...
烧录软件目录中的“最终代码”。
现在，屏幕会根据外界的光照强度改变自己的亮度，总共有四个亮度等级。同时，改变磁传感器的朝向也会改变屏幕上方位点的坐标。
